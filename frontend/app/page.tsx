...
const AUDIO_WAIT_TIMEOUT_MS = 10000;
      ...
      (message: VocaMessage) => {
        if (message.type === 'persona_loaded') {
          ...
          setTranscriptEntries(prev => {
            const next = [...prev, { role: 'user', text, language: lang, timestamp: Date.now() }];
          ...
          markSessionStarted();
          setTranscriptEntries(prev => {
            const next = [...prev, { role: 'voca', text, language: lang, timestamp: Date.now() }];
            transcriptEntriesRef.current = next;
            return next;
          });
          setHasLanguageDetection(true);
